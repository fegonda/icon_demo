...
class CNN(object):
    ...
    def initialize(self):
        ...
        print '#convlayers:', len(self.convLayers)
    ...
    def train_online(self, data):
        ...
        while (elapsed_time < self.trainTime)\
            ...
            #print minibatch_index, minibatch_avg_costs[-1].item(0)

            delta_to_save = time.clock() - save_time
            if n_valid_batches == 0 and minibatch_index%10 == 0:
                
                self.save()
                save_time = time.clock()

            elif (iter + 1) % validation_frequency == 0 and n_valid_batches > 0:
                ...
                print minibatch_avg_costs[-1].item(0), ' - ', this_validation_loss, '/',self.best_validation_loss, len(np.where(probs==0)[0]), len(np.where(probs==1)[0])
